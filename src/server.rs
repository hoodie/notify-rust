...
use dbus::obj::{Argument, Interface, Method, ObjectPath};
use dbus::{BusType, Connection, ConnectionItem, Message, MessageItem, NameFlag};
...
impl NotificationServer {
        ...
        where F: Fn(&Notification)
    {
        ...
        connection.register_name("org.freedesktop.Notifications", NameFlag::ReplaceExisting as u32)
                  .expect("Was not able to register name.");
        let mut objpath = ObjectPath::new(&connection, "/org/freedesktop/Notifications", false);
        connection.register_object_path("/org/freedesktop/Notifications")
        ...
        let server_interface = Interface::new(
            ...
            Method::new(
                ...
                Box::new(|_msg| Ok(vec![MessageItem::new_array(vec!["body".into()]).unwrap()]))
            ),
            Method::new(
                ...
                Box::new(|_msg| {
                    Ok(vec!["notify-rust daemon".into(),
                            "notify-rust".into(),
                            VERSION.into(),
                            "1.1".into()])
                })
            ...
            match n {
                ConnectionItem::MethodCall(mut m) => {
                    if objpath.handle_message(&mut m).is_none() {
                        connection.send(Message::new_error(&m, DBUS_ERROR_FAILED, "Object path not found").unwrap())
                                  .unwrap();
                    }
                }
